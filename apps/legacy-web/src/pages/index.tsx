...
const Home = () => {
  ...
  const sorted = useMemo(() => {
    ...
    let sorted = frontGroups.slice().sort((a, b) => a.rank - b.rank)

    if (sortBy === 'status') {
      sorted = frontGroups.slice().sort(
        (a, b) => (a.active ? 1 : 0) - (b.active ? 1 : 0),
      )
    }

    if (sortBy === 'name') {
      sorted = frontGroups.slice().sort((a, b) => a.name.localeCompare(b.name))
    }

    if (sortBy === 'numAds') {
      sorted = frontGroups.slice().sort((a, b) => a.numAds - b.numAds)
    }

    if (sortBy === 'adSpend') {
      sorted = frontGroups.slice().sort((a, b) => a.adSpendUpper - b.adSpendUpper)
    }
